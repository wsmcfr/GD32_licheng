...
| 下载缓存区 | `0x08070000 ~ 0x0807FFFF` | 暂存“下一版新固件” | App |
...
| 正式 App 区 | `0x0800D000` | `99` 页 | `396KB` (`0x63000`) | 当前工程真正运行的 App |
| 下载缓存区 | `0x08070000` | `16` 页 | `64KB` | OTA 临时缓存 |
...
| `0x0800D000 + 0x63000` | `0x08070000` | 正式 App 区按当前链接配置结束在这里 |
| `0x08080000 - 64KB` | `0x08070000` | 从整片 Flash 末尾倒推 64KB，得到下载缓存区起点 |
...
> 当前工程真正生效的正式 App 区大小是 `0x63000 = 396KB`。  
...
| 区域 | 本质角色 | 当前大小 |
|---|---|---:|
| 正式 App 区 | 设备最终运行程序的长期空间 | `396KB` |
| 下载缓存区 | 升级过程中的临时中转仓库 | `64KB` |

它们不一样大，是因为设计目标本来就不一样：

| 设计点 | 为什么这样做 |
|---|---|
| 正式 App 区要尽量大 | 这里决定“设备最多能运行多大的程序”，所以给了 `0x63000` 的运行空间 |
| 下载缓存区只需满足当前 OTA 方案 | 当前是内部 Flash 两阶段升级，只给下载缓存区留了 `64KB` 作为临时仓库 |
...
| 问题 | 当前答案 |
|---|---|
| 这块板子最多能运行多大的 App？ | 按当前链接地址，正式运行区上限是 `396KB` |
| 当前这套串口 OTA 最多能升级多大的 App？ | 只能升级 `<= 64KB` 的 `Project.bin` |

也就是说：

> “能运行多大 App”和“当前 OTA 能升级多大 App”不是同一个问题。  
> 现在的瓶颈不在正式 App 区，而在下载缓存区只有 `64KB`。
...
| 继续走当前这套内部 Flash 串口 OTA | 只有 `Project.bin <= 64KB` 时可以 | 下载缓存区只有 `64KB`，超过后 START 阶段就会因超长被拒绝 |
...
| 4 | App | 每收一帧就写到 `0x08070000 + offset` | 新固件逐步写入下载缓存区 |
...
| 9 | BootLoader | 把 `0x08070000` 搬到 `0x0800D000` | 新固件变成正式 App |
...
| 4 | 检查固件大小是否超过 `64KB` | 下载缓存区上限就是 `64KB` |
| 5 | 擦除下载缓存区 | 准备接收新固件 |
...
| 5 | 立即写 `0x08070000 + offset` | 不占用大 RAM，支持低内存 OTA |
...
| 问题 | 正确答案 |
|---|---|
| App 有没有把新固件写到 `0x0800D000`？ | 没有。App 只写 `0x08070000` 下载缓存区。 |
