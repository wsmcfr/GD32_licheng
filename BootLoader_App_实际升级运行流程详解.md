...
| 预留间隙 | `0x08070000` | `3` 页 | `12KB` | 当前升级流程未使用，按现有常量推断为保留间隙 |
| 下载缓存区 | `0x08073000` | `13` 页 | `52KB` | OTA 临时缓存 |
...
| `0x08073000 - 0x08070000` | `12KB` | App 区与下载缓存区之间目前留了 3 页间隙 |
| `0x08080000 - 52KB` | `0x08073000` | 从整片 Flash 末尾倒推 52KB，得到下载缓存区起点 |
...
| 下载缓存区只需满足当前 OTA 方案 | 当前是内部 Flash 两阶段升级，只给下载缓存区留了 `52KB` 作为临时仓库 |
...
| 当前这套串口 OTA 最多能升级多大的 App？ | 只能升级 `<= 52KB` 的 `Project.bin` |

也就是说：

> “能运行多大 App”和“当前 OTA 能升级多大 App”不是同一个问题。  
> 现在的瓶颈不在正式 App 区，而在下载缓存区只有 `52KB`。
...
| 继续走当前这套内部 Flash 串口 OTA | 不可以 | 下载缓存区只有 `52KB`，START 阶段就会因超长被拒绝 |
