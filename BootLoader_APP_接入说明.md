...
| 下载缓存区 | `0x08070000` | `64KB` | App 接收 DATA 分包后，先把新固件按偏移写到这里 |
...
| 下载缓存区 | 放在 Flash 尾部 `0x08070000 ~ 0x0807FFFF` | 升级时先写缓存区，避免先破坏旧 App |
...
| 当前这套串口 OTA“最多能升级多大的 App” | 下载缓存区上限是 `64KB` |
...
| 项目 | 当前限制 |
|---|---|
| 在线 OTA 固件大小 | `Project.bin` 必须 `<= 64KB` |
| 超过 `64KB` 后怎么办 | 需要继续重做下载缓存规划，或改成外部 Flash / SD 卡升级 |
...
| `USER/App/uart_ota_app.c` | 新增 `USART2` START/DATA/END 分包 OTA、CRC 校验、下载缓存区写入和参数区写入 | App 每收到一个 OTA DATA 帧就写 `0x08070000`，最后写 `0x0800C000` 并复位交给 BootLoader 搬运 |
| `USER/Driver/bootloader_port.c/.h` | 新增 BootLoader 交接层封装 | 统一管理共享地址、CRC32、向量表校验、下载区擦写、参数区回写和软件复位 |
| `tools/make_uart_ota_packet.py` | 根据 `Project.bin` 生成旧 `.uota` 包，或通过 `--mode send` 按 ACK 分包发送 | 推荐使用 `--mode send --port COMx --baudrate 460800`，避免 App 侧占用 64KB 级 RAM 缓冲，并可观察发送进度 |
...
| 搬运新固件 | 从 `0x08070000` 读，写入 `0x0800D000` | 把下载缓存区的新 App 复制到 App 区 |
...
| 运行中升级 App | 当前工程已支持 USART2 ACK 分包升级 | App 通过 `USART2` 接收 START/DATA/END 帧，边收边写 `0x08070000`，最后写参数区并复位交给 BootLoader 搬运 |
...
| 下载缓存区 | `0x08070000` 起 64KB | 保留 64KB 上限检查 | 当前加入 LittleFS 后的 App 仍可放入缓存区；后续 App bin 必须小于 64KB，除非继续重做下载缓存规划 |

## 8. 当前 BootLoader 搬运逻辑

| 步骤 | 当前实现 |
|---|---|
| 大小检查 | `appSize` 必须大于 0，且不能超过下载缓存区 `64KB` 和 App 区上限 |
| 地址检查 | 目标 App 地址固定要求为 `0x0800D000` |
| 擦除 App 区 | `erase_pages = (appSize + 4095) / 4096`，按 4KB 页擦除 |
| 分块复制 | 每次搬运 `1024B`，从 `0x08070000` 写到 `0x0800D000` |
...
| 串口接收新固件、写 `0x08070000` 缓存区 | 已实现 | `uart_ota_app.c` 通过 `USART2` 分包接收 DATA 帧后立即写下载缓存区，并用 CRC32 校验写入结果 |
...
| 4 | App 每收到一个 DATA 帧就写入下载缓存区 `0x08070000`，最后 END 阶段读回计算 CRC32 |
...
限制说明：当前实现仍使用内部 Flash 下载缓存区，单个 `Project.bin` 必须不超过 `64KB`。如果后续 App 超过该大小，需要继续重新规划下载缓存区，或改成外部 Flash/SD 卡分包升级。
