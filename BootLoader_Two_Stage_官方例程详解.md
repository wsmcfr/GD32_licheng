...
| 阶段 | 执行者 | 主要工作 |
|---|---|---|
| 第一阶段 | 当前 App | 串口接收升级包，校验升级包头，把真正 App 镜像写入下载缓存区 `0x08070000`，再把升级标志、固件大小、CRC 等信息写入参数区 `0x0800C000`，最后软件复位 |
...
| 下载缓存区 | `0x08070000` | 64KB | App 接收升级包后，先把新镜像写到这里 |
...
| 3 | 从下载缓存区 `0x08070000` 读取 `appSize` 字节到 RAM 缓冲 |
