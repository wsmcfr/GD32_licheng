...
/*
 * 文件作用：
 *   定义 USART2 专用 OTA 通道的共享缓冲区、状态复位接口和周期任务入口。
 * 说明：
 ...
 *   定义 OTA 应用层接收缓冲区长度。
 * 说明：
 *   OTA 数据现在专门从 USART2 进入，因此这里直接跟随 Driver 层
 *   USART2 DMA 缓冲长度，保证一个 DATA 帧能够完整放入。
 */
#if defined(BSP_USART2_RX_BUFFER_SIZE)
#define UART_OTA_FRAME_BUFFER_SIZE     BSP_USART2_RX_BUFFER_SIZE
#else
...
/*
 * 变量作用：
 *   USART2 IDLE 中断移交给 OTA 模块的一帧数据、有效长度和完成标志。
 * 说明：
 *   OTA 模块使用独立共享缓冲区，避免和 USART0 普通桥接数据互相覆盖。
 */
...
/*
 * 函数作用：
 *   向 USART2 OTA 专用口发送一次上电探测串，便于现场确认 OTA 线缆和串口号。
 * 参数说明：
 *   无参数。
 * 返回值说明：
 *   无返回值。
