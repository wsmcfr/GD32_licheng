...
/*
 * 函数作用：
 *   显式开启 YModem 接收请求窗口，让后续周期维护可以发送 'C' 请求。
 * 主要流程：
 *   1. 仅在当前未接收文件时生效，避免正在升级时被重复命令打断。
 *   2. 打开短时间轮询窗口，并让下一次轮询立即发出 CRC 请求字符。
 * 参数说明：
 *   无参数。
 * 返回值说明：
 *   无返回值。
 * 说明：
 *   复位后的空闲时不会周期发送 'C'；现场需要先发送文本命令 YMODEM，
 *   再选择 YModem 文件发送，避免不升级时串口助手持续刷 C。
 */
